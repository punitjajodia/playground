...
(function(WATERFALL) {
    ...
    var SPH = (function() {
        function SPH(numX, numY, numZ, options) {
            ...
            this.mViscosityConstant = options.viscosityConstant || 1.1;    // mew
            this.mGasConstant = options.gasConstant || 10; //k
            this.mDiffuseConstant = options.diffuseConstant || 3000.0;   //c
            this.mIdealGasConstant = options.idealGasConstant || 3000;  //alpha
            this.mSmoothingLength = options.smoothingLength || 1.05;    //h or l
            this.mSurfaceTensionCoefficient = options.tensionForceConstant || 500; //sigma
            this.mSurfaceTensionThreshold = options.surfaceTensionThreshold || 0.8;    //beta
            this.mRestitutionCoefficient = options.restitutionCoefficient || 0.2;
            this.mSurfaceColorCoefficient = options.surfaceColorCoefficient || 0.001;   //cs
            this.mVelocityCorrectionFactor = options.velocityCorrectionFactor || 0.005;   //eta
            ...
            for (var z = 0; z < numZ; z = z + 1) {
                for (var y = 0; y < numY; y = y + 1) {
                    for (var x = 0; x < numX; x = x + 1) {
                        ...
                        var newY = y;
                        var newZ = z;
                        this.mParticles.push(this.addParticle(newX, newY, newZ, this.mRestDensity, i));
                        i++;
                    }
                }
            }
        ...
        SPH.prototype.addParticle = function(x, y, z, index) {
            ...
            var particle = new WATERFALL.Particle(x, y, z, index);
...
//                            .multiplyScalar((particle.mPressure / Math.pow(particle.mDensity, 2)) + (neighbor.mPressure / Math.pow(neighbor.mDensity, 2))));
                ...
                // Pressure force
                particle.mForce.add(pressure.multiplyScalar(-1));
                // Viscosity force
        ...
        SPH.prototype.integrate = function() {
            for(var i = 0; i < this.mParticles.length; i++)
            {
                ...
                particle.mPosition.add(newVec().multiply(particle.mVelocity).multiplyScalar(this.mTimeStep) //Current Velocity
                    .add(newVec().multiply(lastAcceleration).multiplyScalar(this.mTimeStep * this.mTimeStep / 2)));
